...
namespace Singletons
{
    public class MessageManager : MonoBehaviourSingleton<MessageManager>
    {
        ...
        {
            Message msg = InternalShowMessage(message, type);
            msg.Display(message, type);
        }

        public void ShowMessage(string message, Func<bool> condition, MessageType type = MessageType.Default) 
        {
            Message msg = InternalShowMessage(message, type);
            msg.Display(message, condition, type);
        }
    
        public void ShowMessage(string message, float duration, MessageType type = MessageType.Default) 
        {
            Message msg = InternalShowMessage(message, type);
            msg.Display(message, duration, type);
        }

        private Message InternalShowMessage(string message, MessageType type = MessageType.Default)
        {
            CreatePool();
            Message msg;
